...
type UnsafeLinkBuffer struct {
	...
	// buf allocated by Next when cross-linkBufferNode, which should be freed when release
	caches [][]byte
...
var featureAlwaysNoCopyRead = false
...
func (b *UnsafeLinkBuffer) readBinary(n int) (p []byte) {
	...
	if b.isSingleNode(n) {
		...
		if !b.read.getMode(readonlyMask) {
			...
			featureAlwaysNoCopyRead && n >= minReuseBytes {
...
func (b *UnsafeLinkBuffer) GetBytes(p [][]byte) (vs [][]byte) {
	...
	if len(p) == 0 {
		...
		//if flush.off > 0 {
		if flush != nil && len(flush.buf[flush.off:]) > 0 {
			n++
		}
		//n++
		//}
